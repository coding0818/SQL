...
#실습 4-1
create table `Member` (`uid` varchar(10) primary key,
...
create table `Department`(`dpeNo` int primary key, 
`name` varchar(10) not null,
`tel` varchar(12) not null
);
create table `Sales`(`seq` int auto_increment primary key ,
...
select * from `Sales` where `sale` >= 50000 and 100000;
...
select substirng(`hp`, 10, 4) as `휴대폰 끝자리` from `Member`;
...
select sum(`sale`) as `총합`,
	   avg(`sale`) as `평균`
from `Sales` 
where `year`=2018 and `month`=2 and `sale`>=50000;
...
select * from `Member` inner join `Department` on `Member`.`dep`=`Department`.`dpeNo`;

select * from `Sales` as a join `Member` as b on a.uid=b.uid;
select * from `Member` as a join `Department` as b on a.dep=b.dpeNo;

select * from `Sales` as a, `Member` b where a.uid = b.uid;
select * from `Member` as a, `Department` as b where a.dep=b.dpeNo;
...
join `Member` as b on a.uid = b.uid
join `Department` as c on b.dep = c.dpeNo;
